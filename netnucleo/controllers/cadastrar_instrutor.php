...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $nome = trim((string)($_POST['nomeInstrutor'] ?? ''));
    $cpf = preg_replace('/\D+/', '', (string)($_POST['cpfInstrutor'] ?? '')) ?: '';
    $email = trim((string)($_POST['emailInstrutor'] ?? ''));
    $telefone = preg_replace('/\D+/', '', (string)($_POST['telefoneInstrutor'] ?? '')) ?: '';
    $area = trim((string)($_POST['areaInstrutor'] ?? ''));

    if ($nome === '') {
        $erro = 'O nome do instrutor é obrigatório.';
    } elseif ($email !== '' && filter_var($email, FILTER_VALIDATE_EMAIL) === false) {
        $erro = 'E-mail inválido.';
    } elseif ($cpf !== '' && preg_match('/^\d{11}$/', $cpf) !== 1) {
        $erro = 'CPF inválido. Informe 11 dígitos.';
    } elseif ($telefone !== '' && preg_match('/^\d{10,11}$/', $telefone) !== 1) {
        $erro = 'Telefone inválido. Informe 10 ou 11 dígitos.';
    } else {
        try {
            if ($cpf !== '') {
                $check = $pdo->prepare('SELECT COUNT(*) FROM instrutor WHERE cpfInstrutor = :cpf');
                $check->execute([':cpf' => $cpf]);
                if ((int)$check->fetchColumn() > 0) {
                    throw new RuntimeException('Este CPF já está cadastrado para outro instrutor.');
                }
            }

            $stmt = $pdo->prepare(
                'INSERT INTO instrutor
                 (nomeInstrutor,cpfInstrutor,emailInstrutor,telefoneInstrutor,areaInstrutor,statusInstrutor)
            ...
            $mensagem = 'Instrutor cadastrado com sucesso. ID: ' . $pdo->lastInsertId();
        } catch (PDOException $e) {
            $erro = 'Não foi possível cadastrar o instrutor. Verifique se o CPF já existe e se os dados são válidos.';
        } catch (Throwable $e) {
            $erro = $e->getMessage();
        }
    }
